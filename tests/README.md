...
└── tests
    ...
    |   ├── path.py                             -> Resource and output path relevant methods
    |   ├── non_functional.py                   -> Non-functional test helper methods
    |   └── evaluation.py                       -> Test evaluation helper methods
    ├── external
    ...
    :
    ├── <module_N>
    ├── test_commonroad_interactive_benchmarks  -> Contains INTEGRATION tests
...
Example:
```bash
pytest tests --scope unit module --type functional -m "not slow"
```
