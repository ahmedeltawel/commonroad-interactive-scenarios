...
###
# plot the trajectories of one vehicle in each simulation
###
list_position_x = []
list_position_y = []
vehicle_id_list = []
...
vehicle_id_test = 335  # set an interesting vehicle id manually

plt.figure(figsize=(10, 10))
#plt.title('Trajectory of vehicle ' + str(vehicle_id_test) + ' in ' + str(n+1) + 'th simulation', fontsize=12, color='k')
draw_object(list_simulated_scenario[0], draw_params={'time_begin': -1, 'time_end': -1})
...
plt.margins(0, 0)
plt.savefig(os.path.join(video_output_folder, conf.scenario_name + '.png'), format='png', dpi=300)
